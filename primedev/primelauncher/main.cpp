...
void LibraryLoadError(DWORD dwMessageId, const wchar_t* libName, const wchar_t* location)
{
	...
	swprintf_s(
		text,
		L"无法读取文件 %ls 于 \"%ls\" (%lu):\n\n%hs\n\n请检查安装流程是否正确,或在北极星CN Wiki中查询常见疑难问题解答",
		libName,
		location,
		dwMessageId,
		message.c_str());
	...
	if (dwMessageId == 126 && std::filesystem::exists(location))
	{
		swprintf_s(
			...
			L"运行库: https://aka.ms/vs/17/release/vc_redist.x64.exe \n2. 在您安装游戏的平台中校验游戏完整性",
			text);
	}
	...
	else if (!fs::exists("Titanfall2.exe"))
	{
		swprintf_s(
			text,
			L"%s\n\n请注意: "
			L"您需要将北极星CN安装包解压到游戏安装目录，与'Titanfall2.exe'同级的文件夹中，而不是直接在压缩包内运行或解压到任意位置！",
			text);
	}
	else if (fs::exists("Titanfall2.exe"))
	{
...
int main(int argc, char* argv[])
{
	...
	if (!LauncherMain)
		MessageBoxW(GetForegroundWindow(), L"无法找到 launcher.dll.\n启动游戏时失败", L"错误", 0);
